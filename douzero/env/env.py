...
# 添加癞子(0)和问号(1)，各4张，花色用-1表示
for _ in range(4):
    deck.append(((0, -1), card_ids))  # 4张癞子
    card_ids += 1
for _ in range(4):
    deck.append(((1, -1), card_ids))  # 4张问号
    card_ids += 1

# 添加8，每个花色三张
for suit in range(4):
    for _ in range(3):
        deck.append(((8, suit), card_ids))  # 每个花色3张8
        card_ids += 1

# 添加其他点数牌，每个花色两张
for point in [6, 7, 9, 10, 11, 12, 13, 14]:
    for suit in range(4):
        for _ in range(2):
            deck.append(((point, suit), card_ids))  # 每个花色2张
            card_ids += 1
...
# 获取牌的花色符号
#get_suit_symbol = lambda card: CARD_SUIT[card[1]]
...
class Env:
    ...
    def reset(self):
        ...
        card_play_data =  {
                            ...
                            'up': _deck[14:21],
                            'left': _deck[21:28],
                            'public': [_deck[28:29], None, None, None],
                            'remaining': _deck[29:],
                            'discard': []
                          }
